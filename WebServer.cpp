...
// Custom port
WebServer::WebServer(int port): port(port), webServer(port) { //Service("webserver: "+std::to_string(SERVER_PORT)) {
 ...
 WebServer *WebServer::instance(int port) {
  ...
  if (auto search = WebServer::instances->find(port); search != WebServer::instances->end()){
    std::cout << "existing service found" << std::endl;
  }else{
    std::cout << "no existing webserver found on port " << port <<  "=> creating" << std::endl;
...
void WebServer::start() {
  if (!serverState) {
    std::cout << "[WebServer:" << port << "] First instance : starting Server" << std::endl;
    // WebServer<SERVER_PORT>::server = new AsyncWebServer(SERVER_PORT);
    webServer.begin();
    serverState = true;
    std::cout << "[WebServer:" << port << "] Server running on port "<< port << std::endl;
  }
}